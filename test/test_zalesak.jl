...
# 2. Define an evolving level set (circle advected by constant velocity)
...
println("=" ^ 70)
println("       ZALESAK DISK EVOLUTION TEST - PERFORMANCE REPORT")
println("=" ^ 70)
...
if lsm_available
    ...
    # INITIAL LEVEL SET / ZALESAK DISK
    # Using proper CSG: Zalesak = Circle \ Rectangle
    # Formula: ϕ_{A\B} = max(ϕ_A, -ϕ_B)

    # Zalesak disk parameters
    cx, cy = -0.75, 0.0   # Disk center
    R = 0.5               # Disk radius
    slot_w = 0.2          # Slot width
    slot_h = 0.5          # Slot height

    # Analytical signed distance function for Zalesak disk
    function ϕ0(x)
        # SDF for circle (negative inside, positive outside)
        d_circle = sqrt((x[1] - cx)^2 + (x[2] - cy)^2) - R

        # SDF for slot rectangle
        slot_cx, slot_cy = cx, cy - R + slot_h/2
        hw, hh = slot_w/2, slot_h/2

        dx = abs(x[1] - slot_cx) - hw
        dy = abs(x[2] - slot_cy) - hh
        outside_dist = sqrt(max(dx, 0)^2 + max(dy, 0)^2)
        inside_dist = min(max(dx, dy), 0.0)
        d_rect = outside_dist + inside_dist

        return max(d_circle, -d_rect)
    end
    ...
    grid = GridInfo((-2.0, -2.0), (2.0, 2.0), (100, 100))
    ...
    t_save = @elapsed save_all(rec, "evolution_"; fps=5)
    ...
    println("  - evolution_levelset.gif (2D contour)")
    println("  - evolution_levelset3d.gif (3D surface)")
    println("  - evolution_grid.gif (grid with IN/OUT/CUT cells)")
    println("  - evolution_cutmesh.gif (physical domain triangulation)")
    
    # =========================================================================
    # SAVE METADATA
    # =========================================================================
    metadata = Dict(
        "test_name" => "zalesak_disk",
    ...
    metadata_file = "zalesak_perf_" * Dates.format(now(), "yyyymmdd_HHMMSS") * ".json"
