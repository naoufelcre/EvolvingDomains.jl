...
function run_csg_tutorial()
    ...
    geom_zalesak = EvolvingDiscreteGeometry(model, x -> geo_zalesak(x))
    geom_clipped = EvolvingDiscreteGeometry(model, x -> geo_clipped(x))
    geom_union   = EvolvingDiscreteGeometry(model, x -> geo_union(x))
    ...
    save("EvolvingDomainsDocumentation/assets/csg_overview.png", fig)
    println("Saved EvolvingDomainsDocumentation/assets/csg_overview.png")
    
    return fig
end

# Check if GLMakie is loaded effectively (interactive mode)
